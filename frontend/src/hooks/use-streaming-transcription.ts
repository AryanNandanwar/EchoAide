    ...
    try {
      await withSpan(
        ...
        async (span) => {
          ...
          recordingSessionStarted.add(1, { outcome: 'success' });
      ...
      recordingSessionStarted.add(1, { outcome: 'error' });
  ...
  const stopRecording = useCallback((noteId?: string, doctorId?: string, options: StopRecordingOptions = {}) => {
    void withSpan(
      ...
      async (span) => {
        ...
        if (noteId) {
          span.setAttributes({ 'note.id': noteId });
        }
        ...
        if (wsRef.current && sessionId) {
          ...
          console.log("🆔 Session ID being stopped:", sessionId);
          console.log("📋 Additional parameters:", { noteId, doctorId, ...options });

          if (!doctorId) {
            console.error("❌ Doctor ID is required for new clinical note flow");
            recordingSessionStopped.add(1, { outcome: 'error', reason: 'no_doctor_id' });
            span.setAttributes({ outcome: 'error', 'error.type': 'no_doctor_id' });
            setState(prev => ({
              ...prev,
          ...
          const finalNoteId = wsRef.current.stopRecording(sessionId, doctorId, noteId, options);
        ...
        recordingSessionStopped.add(1, { outcome: 'success' });
        span.setAttributes({ outcome: 'success' });
